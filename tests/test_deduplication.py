...
merge_close_points_global semantics under test:
  ...
  - the merged peak is the max peak in the cluster, and the adjusted peak and
    raw adjustment signal are that same member's (not the max of each), so the
    three scores of a merged point stay consistent with one another,
...
import pytest
from _helpers import DEG_PER_M, EARTH_RADIUS_M, predicted_point as point
...
def test_merge_two_points_within_threshold_averages_position():
    ...
    dlat = 1.0 * DEG_PER_M
    flat = [point(0.0, 10.0, 0.5, 0.7), point(dlat, 10.0, 0.9, 0.3)]
...
def test_merged_adjusted_peak_follows_the_max_peak_member():
    ...
    assert len(result) == 1
    peak, adj = result[0].peak_value, result[0].adjusted_peak
    assert peak == pytest.approx(0.9, abs=1e-12)
    assert adj == pytest.approx(0.3, abs=1e-12)


def test_merged_point_keeps_the_max_peak_members_three_scores_together():
    # Given: two mergeable points whose scores would disagree if mixed — the
    #        max-peak member (peak=0.9) carries adj=0.95 and signal=0.05, the
    #        other the larger adj=1.2 and signal=0.7
    dlat = 1.0 * DEG_PER_M
    flat = [
        point(0.0, 10.0, 0.5, 1.2, 0.7),
        point(dlat, 10.0, 0.9, 0.95, 0.05),
    ]

    # When: merge_close_points_global runs with min_distance_m=2
    result = merge_close_points_global(flat, min_distance_m=2.0)

    # Then: all three scores come from that one member, so the merged point
    #       still satisfies adjusted_peak == peak_value + 1.0 * signal
    assert len(result) == 1
    merged = result[0]
    assert merged.peak_value == pytest.approx(0.9, abs=1e-12)
    assert merged.adjusted_peak == pytest.approx(0.95, abs=1e-12)
    assert merged.adjustment_signal == pytest.approx(0.05, abs=1e-12)
    assert merged.adjusted_peak == pytest.approx(
        merged.peak_value + merged.adjustment_signal, abs=1e-12
    )


def test_merged_point_of_non_positive_peaks_is_a_real_member():
    # Given: a cluster whose members all have non-positive peaks
    dlat = 1.0 * DEG_PER_M
    flat = [
        point(0.0, 10.0, -0.5, -0.4, 0.1),
        point(dlat, 10.0, -0.2, -0.1, 0.1),
    ]

    # When: merge_close_points_global runs with min_distance_m=2
    result = merge_close_points_global(flat, min_distance_m=2.0)

    # Then: the merged scores are the least-negative member's (-0.2), not the
    #       zeros a max-seeded-at-0.0 accumulator would emit for a point that
    #       no member ever had
    assert len(result) == 1
    assert result[0].peak_value == pytest.approx(-0.2, abs=1e-12)
    assert result[0].adjusted_peak == pytest.approx(-0.1, abs=1e-12)


def test_points_beyond_threshold_stay_separate():
    # Given: two points 3 m apart along a meridian, min_distance_m=2
    dlat = 3.0 * DEG_PER_M
    flat = [point(0.0, 10.0, 0.5, 0.5), point(dlat, 10.0, 0.6, 0.6)]
    ...
    assert len(result) == 2
    lats = sorted(p.lat for p in result)
...
def test_merge_radius_separates_just_inside_from_just_outside():
    ...
    merged_inside = merge_close_points_global(
        [point(0.0, 10.0, 0.5, 0.5), point(inside * DEG_PER_M, 10.0, 0.6, 0.6)],
        min_distance_m=2.0,
    )
    merged_outside = merge_close_points_global(
        [point(0.0, 10.0, 0.5, 0.5), point(outside * DEG_PER_M, 10.0, 0.6, 0.6)],
        min_distance_m=2.0,
    )
...
def test_merging_is_transitive_through_chains():
    ...
    d = DEG_PER_M
    flat = [
        point(0.0, 10.0, 0.1, 0.1),
        point(1.5 * d, 10.0, 0.2, 0.2),
        point(3.0 * d, 10.0, 0.3, 0.3),
    ]
    ...
    assert len(result) == 1
    lat, lon, peak = result[0].lat, result[0].lon, result[0].peak_value
...
def test_agreement_two_drops_singletons_but_keeps_pairs():
    ...
    d = DEG_PER_M
    flat = [
        point(0.0, 10.0, 0.5, 0.5),
        point(1.0 * d, 10.0, 0.7, 0.6),
        point(100.0 * d, 10.0, 0.9, 0.9),
    ]
    ...
    assert len(result) == 1
    lat, peak = result[0].lat, result[0].peak_value
...
def test_agreement_larger_than_every_cluster_returns_empty():
    ...
    d = DEG_PER_M
    flat = [point(0.0, 10.0, 0.5, 0.5), point(1.0 * d, 10.0, 0.7, 0.6)]
...
def test_exact_duplicates_merge_even_with_zero_distance():
    ...
    flat = [point(5.0, 6.0, 0.4, 0.4), point(5.0, 6.0, 0.8, 0.7)]
...
def test_two_distinct_clusters_each_merge_independently():
    ...
    flat = [
        point(0.0, 10.0, 0.1, 0.1),
        point(1.0 * d, 10.0, 0.2, 0.2),
        point(100.0 * d, 10.0, 0.3, 0.3),
        point(101.0 * d, 10.0, 0.4, 0.4),
    ]
